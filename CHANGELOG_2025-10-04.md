...
### Task Organization: Cone Arrow Ability Completed (2025-10-07)
- **Archived**: Moved 2d_ABILITIES_cone_arrow_ability.md to completed-tasks/data-systems/
- **Status**: Task complete - volley.tres exists with cone spread (no homing)
- **Implementation**:
  - File created: data/content/abilities/projectile/volley.tres
  - Auto-discovered by AbilityManager (directory scanning pattern)
  - Key properties: is_homing=false, spread_angle=40.0, projectile_count=33
  - Demonstrates declarative ability creation (change 3 fields → new behavior)
- **Architecture**: No manual registration needed - filesystem IS the registry
- **Testing**: Visual testing with 33 projectiles confirms cone spread works correctly

### Performance: Enemy Movement Speed-Up During Lag (2025-10-07)
- **Fixed**: Enemies moving faster than configured speed during stuttering/lag spikes
- **Root Cause**: Delta time accumulation during frame drops
  - Normal frame: dt = 0.033s → enemy moves `speed * 0.033`
  - Lag spike: dt = 0.150s (5 frames) → enemy moves `speed * 0.150` = **4.5x distance in one frame**
  - Result: Enemies appear to "rush", "teleport", or "charge" toward player during stutters
- **Solution**: Clamped delta time to 50ms maximum (3x normal 30Hz frame time)
  - `var clamped_dt = minf(dt, 0.05)` before position update
  - During lag: enemy moves at most `speed * 0.05` instead of `speed * 0.150`
  - **Preserves consistent enemy speed even during performance issues**
- **File**: scripts/systems/spawn/SpawnDirector.gd:901-954
...
- **Solutions Applied**:
  - **Fix 0 (PRIMARY FIX)**: Spawn rate limiting with pending queue to prevent frame spikes
    - Limited enemy spawns to 10 per 30Hz frame (was unlimited - 20-45 per ring × 3 breaches)
    - Added `pending_ring_spawns` counter to track unfinished rings across frames
    - `should_spawn_new_ring()` continues spawning pending enemies before starting new rings
    - **Guarantees original enemy count** - just spreads spawning across multiple frames
    - **Prevents synchronous scene tree additions from blocking the main thread**
    - Performance: 200+ spawns in 1 frame → 10 spawns per frame × 20 frames (smooth)
    - Example: 45-enemy ring spawns as 10+10+10+10+5 across 5 frames (0.16 seconds @ 30Hz)
  - **Fix 0b**: Eliminated redundant `get_alive_enemies()` calls during spawn
  ...
  - **Fix 4**: Performance profiling clarity - fixed logging logic errors
    - Removed duplicate DEBUG format logs that were overriding performance profiling
    - Fixed backward condition (was `if elapsed_ms > 0.0` instead of `> 5.0`)
    - Added debug log to verify function execution with spawn counts
    - Now shows clear timing format: `"Ring spawn: X ms for Y enemies (breach breach_id)"`
    - WARN threshold at 5ms for slow operations identification
  - **Fix 5**: Comprehensive system profiling to identify actual bottleneck
    - **All logs unified under "LAG" category** for easy filtering
    - Added profiling to SpawnDirector._on_combat_step() (TOTAL SpawnDirector frame time)
      - WARN threshold: >16ms (half of 30Hz frame budget)
      - INFO threshold: >3ms
    - Added profiling to SpawnDirector._update_enemies() (enemy AI pathfinding)
      - Logs when >0 enemies processed AND >0ms (or >8ms for WARN)
      - WARN threshold: >8ms
    - Added profiling to DamageSystem._check_enemy_player_collisions() (collision detection)
      - Logs when >0 enemies checked AND >0ms (or >8ms for WARN)
      - WARN threshold: >8ms
    - Added spawn failure tracking to identify zone validation issues
      - Logs zone_failures and spawn_failures counts per ring spawn
      - Helps identify wasted loop iterations during spawn attempts
    - **Critical Finding**: Ring spawn times remain consistent during lag (0.9-4.3ms), spawn success rate is high (spawned=requested)
    - **SpawnDirector profiling**: Total combat step time 3-7.5ms (reasonable for 30Hz frame budget of 33ms)
    - Added profiling to AbilityController._on_combat_step() for projectile/ability processing
      - WARN threshold: >8ms, INFO threshold: >3ms
    - Added profiling to BossUpdateManager._on_combat_step() for boss AI batch processing
      - WARN threshold: >8ms, INFO threshold: >3ms
    - Added profiling to MeleeSystem._on_combat_step() for auto-attack processing
      - WARN threshold: >8ms, INFO threshold: >3ms
    - Added profiling to BreachEventHandler._on_combat_step() for breach lifecycle updates (non-spawn)
      - WARN threshold: >8ms, INFO threshold: >3ms
    - **All major combat_step listeners now profiled under "LAG" category**
    - **BOTTLENECK IDENTIFIED**: BossUpdateManager with 440-471 bosses consuming 6.7-8.0ms per frame (24% of 33ms budget)
      - Root cause: Breach enemies being registered as "bosses" in BossUpdateManager
      - Problem: All 450+ bosses updated every frame with `_update_ai_batch()` calls
      - Impact: Combined with SpawnDirector (3-7ms), using 50-70% of frame budget on AI alone
    - **ACTUAL ROOT CAUSE FOUND**: 1-2 second freeze from synchronous `add_child()` operations
      - Spawning 10 enemies/frame × 3-5 breaches = 30-50 synchronous scene tree additions per frame
      - Each `add_child()` takes ~30-50ms when scene tree is large → 900-2500ms total freeze
      - BossUpdateManager (7-8ms) is background cost, not the freeze cause
    - **FIX APPLIED (v1)**: Changed `spawn_container.add_child(enemy_instance)` to `spawn_container.add_child.call_deferred(enemy_instance)`
      - Defers scene tree additions to idle time, preventing main thread blocking
      - Should eliminate 1-2 second freezes during burst spawning
    - **FIX REVISED (v2)**: Removed render_tier check - all spawns deferred
      - **User clarification**: Game spawns 400+ concurrent enemies at start, all are boss scenes
      - No distinction between "boss" and "regular" enemies needed - unified scene-based architecture
      - All enemies use deferred `add_child()` to prevent main thread blocking
    - **FIX REVISED (v3)**: Fixed boss scaling timing for deferred spawning
      - **Problem**: `setup_from_spawn_config()` used `call_deferred()` for sprite scaling (BaseBoss.gd:193)
      - This created race condition: sprite scaling deferred + add_child deferred = wrong order
      - **Solution**: Changed `apply_unified_scaling()` to apply sprite scaling immediately
      - Scaling now happens BEFORE node enters tree (during instantiation phase)
      - Bosses should now have correct size/scale even with deferred `add_child()`
    - **Architecture Note**: All enemies currently spawn as scene-based bosses (no pooled entities)
      - 400+ "bosses" in BossUpdateManager is expected behavior (not a bug)
      - All spawn via `_spawn_boss_scene()` which creates BaseBoss instances
      - BossUpdateManager consuming 7-8ms per frame for 450+ bosses is secondary optimization target
      - TODO: Optimize boss AI batch processing for high enemy counts (e.g., spatial culling, staggered updates)
    - **Profiling Added (v3)**: Track BaseBoss initialization and registration performance
      - Added microsecond-level profiling to `BaseBoss._ready()` (lines 58-140)
      - Logs if individual boss init takes >1ms (INFO) or >5ms (WARN)
      - Added batch registration logging to BossUpdateManager (every 50 bosses)
      - Helps identify which initialization steps are slow during 400+ boss spawns
      - Focus: Tween creation (400 tweens), signal connections (1200 connections), entity registration (800 registrations)
    - **ACTUAL BOTTLENECK FOUND (v4)**: Physics Frame Time at 16.66ms for 400+ enemies
      - **Profiler analysis**: Physics Frame Time 16.66ms (50% of 33ms budget), Physics Time 5.09ms
      - **Root cause**: Personal space Area2D system checking 400 bosses against each other
      - **O(n²) complexity**: 400 × 400 / 2 = 160,000 potential collision checks per physics frame
      - Each boss has CharacterBody2D + Area2D personal space = 800+ physics bodies at 60 Hz
      - Combat systems only using 10-14ms (30-42% budget) - very efficient
      - **FIX APPLIED**: Disabled personal space Area2D system for high enemy counts
        - Commented out `_setup_personal_space_area()` in `BaseBoss._ready()`
        - Commented out `apply_personal_space_forces()` in `_update_ai()`
        - Eliminates 400 Area2D nodes and associated O(n²) collision checks
        - **Expected result**: Physics Frame Time drops from 16.66ms to <5ms
      - **Trade-off**: Enemies may overlap more (no boss-to-boss spacing)
      - **TODO**: Re-implement with spatial partitioning or distance-based culling for scalability
    - **Process Time Investigation (v5)**: Added sprite visibility test for 31ms Process Time spike
      - **New finding**: Process Time at 31ms (94% of frame budget) is the real bottleneck
      - **Test added**: `animated_sprite.visible = false` to isolate rendering cost
      - If Process Time drops to <10ms → AnimatedSprite2D animations are the bottleneck (400 sprites)
      - If Process Time stays at 31ms → Something else in _process() (tweens, UI updates, etc.)
      - **CLEANUP NEEDED**: Remove temporary test code after identifying bottleneck
        - [ ] Remove `animated_sprite.visible = false` test (BaseBoss.gd:65-68)
        - [ ] Consider re-enabling personal space if performance allows
        - [ ] Reduce/remove performance profiling logs (creates overhead at 400+ scale)
    - **Radar System Investigation (v6)**: Testing if radar is causing 31ms Process Time
      - **Finding**: Lag persists even with sprites disabled - NOT a rendering issue
      - **Suspect**: RadarSystem._process() calling get_alive_enemies() at 30 Hz
      - **Test added**: Early return in RadarSystem._process() to disable radar
      - If Process Time drops → Radar scanning 400+ bosses is the bottleneck
      - If Process Time stays at 31ms → Need to check other _process() systems
      - **CLEANUP NEEDED**: Remove radar disable test (RadarSystem.gd:91-93)
- **Files**:
  - scripts/systems/events/BreachEventHandler.gd:333-402, 440-467, 615-645
  - scripts/systems/events/BreachEnemyTracker.gd:67-128
  - scripts/systems/spawn/SpawnDirector.gd:460-488, 791-794, 901-1006
  - scripts/systems/combat/DamageSystem.gd:106-138
  - scripts/systems/combat/MeleeSystem.gd:67-82
  - scripts/systems/boss/BaseBoss.gd:58-148, 135-138, 190-214, 258-263
  - scripts/systems/boss/BossUpdateManager.gd:44-56, 81-143
  - scripts/systems/radar/RadarSystem.gd:90-93
  - scripts/systems/AbilityController.gd:76-92
...
### Performance Investigation - Boss Spawn Lag (2025-10-08) - ONGOING

**Issue:** Process Time spiking to 31ms with 400+ concurrent boss spawns at game start, causing severe lag despite efficient combat systems

**Investigation Process:**
1. ❌ **Sprite rendering test** - Disabled `animated_sprite.visible = false` → No improvement
2. ❌ **Radar system test** - Added early return in RadarSystem._process() → No improvement
3. ❌ **Health bar UI test** - Disabled health bar visibility → No improvement
4. ❌ **Process callback test** - Disabled `set_process(false)` → No improvement
5. ❌ **Physics callback test** - Disabled `set_physics_process(false)` → No improvement
6. ❌ **Process mode test** - Set `process_mode = Node.PROCESS_MODE_DISABLED` → False positive (enemies stopped functioning)

**Current Understanding:**
- 31ms Process Time bottleneck remains **UNIDENTIFIED**
- Systematic testing ruled out: sprites, radar, health bars, _process(), _physics_process(), node processing
- Combat systems confirmed efficient: BossUpdateManager (3-8ms), combat logic (10-14ms)
- Architecture validated: All 400+ enemies as BaseBoss scene instances is correct approach

**Optimizations Implemented (Still Active):**
- ✅ Disabled O(n²) personal space Area2D system (BaseBoss.gd lines 135-138, 258-263)
- ✅ Added performance profiling to BaseBoss initialization (lines 140-148)
- ✅ Reduced BossUpdateManager registration logging to every 50 bosses (lines 54-56)
- ✅ Added physics space validation for move_and_slide() safety (lines 280-283) - reverted as not needed

**Next Investigation Steps:**
- Profile Godot engine internals (VisualServer, PhysicsServer2D)
- Check AnimatedSprite2D animation processing overhead
- Investigate CharacterBody2D collision system scaling
- Consider entity pooling vs scene-based approach
- Test with simplified boss scenes (fewer child nodes)

**All Test Code Reverted:**
- Removed all temporary test modifications
- Codebase returned to clean state with only proven optimizations
