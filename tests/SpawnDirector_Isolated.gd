...
func _setup_wave_system():
	...
	if spawn_director.has_signal("wave_started"):
		spawn_director.wave_started.connect(_on_wave_started)
	if spawn_director.has_signal("wave_completed"):
		spawn_director.wave_completed.connect(_on_wave_completed)
	if spawn_director.has_signal("enemy_spawned"):
		spawn_director.enemy_spawned.connect(_on_enemy_spawned)
	
	# Connect to wave director enemy updates
	if spawn_director.has_signal("enemies_updated"):
		spawn_director.enemies_updated.connect(_on_enemies_updated)
...
func _start_or_next_wave():
	if spawn_director.has_method("start_next_wave"):
		spawn_director.start_next_wave()
		print("Starting next wave")
	elif spawn_director.has_method("start_wave"):
		spawn_director.start_wave()
		print("Starting wave")

func _reset_waves():
	if spawn_director.has_method("reset_waves"):
		spawn_director.reset_waves()
	print("Waves reset")
...
func _test_entity_registration():
	...
	# Spawn several scene enemies manually through SpawnDirector V2 spawn 
	var spawn_count = 5
