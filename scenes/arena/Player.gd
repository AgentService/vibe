...
## Updates ability cooldowns every frame (called in _physics_process).
...
## Auto-casts ready abilities (called in _physics_process).
## For Phase 1, abilities fire automatically when off cooldown.
...
func _activate_ability(slot_index: int) -> void:
	...
	# Context includes player reference, enemy list, and direction
	var context: Dictionary = {
		"player": self,
		"enemies": _get_nearby_enemies(),  # Will be implemented when needed
...
## Returns normalized Vector2 based on last movement or mouse position.
