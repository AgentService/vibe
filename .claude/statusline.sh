...
# Custom Claude Code statusline - Created: 2025-09-19T20:53:22.949Z
...
HAS_JQ=0
# Enhanced detection for Windows environments
if command -v jq >/dev/null 2>&1 || which jq >/dev/null 2>&1 || where jq >/dev/null 2>&1 || [ -x "/c/Windows/System32/jq.exe" ]; then
  HAS_JQ=1
fi
  ...
  if [ -n "$blocks_output" ]; then
    ...
    if [ -n "$active_block" ]; then
      ...
      if [ -n "$reset_time_str" ] && [ -n "$start_time_str" ]; then
        ...
        if [ "$now_sec" -gt "$end_sec" ]; then
          # Reset has occurred, try to fetch fresh data
          session_txt="Reset occurred - fetching new block data..."
          session_bar="=========="
          session_pct=100

          # Try to get fresh blocks data
          fresh_blocks_output=""
          if command -v timeout >/dev/null 2>&1; then
            fresh_blocks_output=$(timeout 3s ccusage blocks --json 2>/dev/null)
          elif command -v gtimeout >/dev/null 2>&1; then
            fresh_blocks_output=$(gtimeout 3s ccusage blocks --json 2>/dev/null)
          else
            fresh_blocks_output=$(ccusage blocks --json 2>/dev/null)
          fi

          if [ -n "$fresh_blocks_output" ]; then
            fresh_active_block=$(echo "$fresh_blocks_output" | jq -c '.blocks[] | select(.isActive == true)' 2>/dev/null | head -n1)
            if [ -n "$fresh_active_block" ]; then
              fresh_reset_time=$(echo "$fresh_active_block" | jq -r '.usageLimitResetTime // .endTime // empty')
              fresh_start_time=$(echo "$fresh_active_block" | jq -r '.startTime // empty')

              if [ -n "$fresh_reset_time" ] && [ -n "$fresh_start_time" ]; then
                fresh_start_sec=$(to_epoch "$fresh_start_time")
                fresh_end_sec=$(to_epoch "$fresh_reset_time")

                # Check if fresh data is actually newer
                if [ "$fresh_start_sec" -gt "$start_sec" ] || [ "$fresh_end_sec" -gt "$end_sec" ]; then
                  # Use fresh data for calculations
                  start_sec=$fresh_start_sec
                  end_sec=$fresh_end_sec
                  ...
                  # Fresh data isn't newer, show that reset occurred
                  elapsed_since_reset=$(( now_sec - end_sec ))
                  if [ "$elapsed_since_reset" -lt 3600 ]; then
                    reset_mins=$(( elapsed_since_reset / 60 ))
                    session_txt="Reset occurred ${reset_mins}m ago"
                  else
                    reset_hours=$(( elapsed_since_reset / 3600 ))
                    session_txt="Reset occurred ${reset_hours}h ago"
                  fi
                  session_bar="=========="
                  session_pct=100
                fi
          ...
          # Normal case: reset time is in the future
          total=$(( end_sec - start_sec )); (( total<1 )) && total=1
          elapsed=$(( now_sec - start_sec )); (( elapsed<0 ))&&elapsed=0; (( elapsed>total ))&&elapsed=$total
          session_pct=$(( elapsed * 100 / total ))
