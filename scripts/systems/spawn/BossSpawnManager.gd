...
func spawn_single_boss_fallback() -> void:
	...
	# Boss config ready for unified scaling system (no hardcoded multipliers)
	Logger.info("4. Boss config prepared...", "debug")
	Logger.info("   Health: " + str(boss_config.health) + " (base)", "debug")
	Logger.info("   Damage: " + str(boss_config.damage) + " (base)", "debug")
...
func spawn_configured_boss(config: BossSpawnConfigScript, spawn_pos: Vector2) -> void:
	...
	# Boss config ready for unified scaling system (no hardcoded multipliers)
	Logger.info("Boss config prepared - Health: " + str(boss_config.health) + " (base)", "debug")
	Logger.info("Boss config prepared - Damage: " + str(boss_config.damage) + " (base)", "debug")
	Logger.info("Boss config prepared - Size: Using variation size_factor (%.1f)" % boss_config.size_scale, "debug")
