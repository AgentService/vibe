...
# Static Analysis Tool for Architecture Boundary Enforcement
# Generates detailed dependency graphs and violation reports
# Can be run from Godot Editor via Tools > Execute Script

class_name ArchitectureBoundaryChecker
...
func _extract_class_dependency(line: String) -> String:
	# Extract class name from extends/class_name declarations
...
func _generate_dot_graph() -> void:
	var output_path := "user://architecture_graph.dot"
	var file := FileAccess.open(output_path, FileAccess.WRITE)
	
	if file == null:
		print("Warning: Could not write dependency graph to " + output_path)
		return
	
	file.store_line("digraph architecture {")
	file.store_line("  rankdir=TB;")
	file.store_line("  node [shape=box];")
	
	# Add layers as subgraphs
	for layer in LAYER_DEFINITIONS:
		file.store_line("  subgraph cluster_" + layer + " {")
		file.store_line("    label=\"" + layer.capitalize() + "\";")
		file.store_line("    color=blue;")
		
		for file_path in file_layers:
			if file_layers[file_path] == layer:
				var node_name: String = file_path.get_file().get_basename()
				file.store_line("    \"" + node_name + "\";")
	...
	print("Use Graphviz to visualize: dot -Tpng " + output_path + " -o architecture.png")
